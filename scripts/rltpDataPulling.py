...
def status_up(desc):
    apt_tool_Db().execute(f"update APT_CUSTOM_POSTBACK_REQUEST_DETAILS_DND set request_status='E',request_desc='{desc}',error_code=1 where request_id={sys.argv[1]}")
    apt_tool_Db().close()
...
if __name__ == "__main__":
    try:
        ...
        p2cur=pg1con.cursor()
        p2cur.execute(f"update APT_CUSTOM_POSTBACK_REQUEST_DETAILS_DND set request_desc='TRT Imported' where  request_id={sys.argv[1]}")

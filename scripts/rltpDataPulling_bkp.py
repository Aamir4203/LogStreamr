...
def status_up(desc):
    ...
    cur=engine.connect()
    cur.execute(f"update APT_CUSTOM_POSTBACK_REQUEST_DETAILS_DND set request_status='E',request_desc='{desc}' where request_id={sys.argv[1]}")
    cur.close()
    engine.dispose()

def main(args):
    client, trt_tb, qr, presto_config, pg_config,sup_l,n,deciles_=args
    try:
        if event.is_set():
           return
        if deciles_=='True':
    ...
    try:
        ...
        with psycopg2.connect(**pg_config) as conn:
            ...
            with conn.cursor() as cursor:
                ...
                df3.columns = ['Delivered', 'Opens', 'clicks', 'unsubs', 'segment', 'sub_seg', 'decile']
            ...
            if re.findall('apt_rltp_request_raw_',qr):
                ...
                try:
                    ...
                    with Pool(processes=5,initializer=init_worker,initargs=(shared_event,)) as pool:
                        pool.map(main, [(client, trt_tb, qr, presto_config, pg_config,sup_l,n,deciles_) for client in decilel])
                    logger.info("All threads for TRT are completed.")
                    '''
                    end_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    logger.info("Ended Time : " + end_time)
                    time1 = datetime.strptime(start_time, "%Y-%m-%d %H:%M:%S")
                    time2 = datetime.strptime(end_time, "%Y-%m-%d %H:%M:%S")
                    total_ex = time2 - time1
                    logger.info(f"Total Execution time: {total_ex}")
                    '''
                except Exception as e:
